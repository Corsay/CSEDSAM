...
sub EncDecLong {
	...
	# 4.1. Если последний элемент помещенный в массив (сообщение или его часть) имеет длину меньше длины ключа, то дополнить сообщение до длины ключа (заранее оговоренными символами).
	...
	for my $i (1..$#msgMas) {
		...
		# ToDo проверяем что ключ не перезаписан(заданной по-умолчанию последовательностью) - в случае если ключ перезаписан, возвращаем undef (ошибка)
		# ToDo перезаписываем использованный ключ(заданной по-умолчанию последовательностью) в таблице(справочнике) ключей
		# для перезаписи лучше сохранять в данном цикле конкатенацию текущего ключа в массив и затирать ключи согласно данному массиву в конце шифрования
	}
	# ToDo доопределить работу с первым справочником (генерация ключа нужной длины)

	# 4.2. обрезаем ключ последнего сообщения до длины сообщения
	# ToDo Реализоввывать либо пункт 4.1 либо данный пункт (обсудить)
	#if (length(unpack "B*", $msgMas[$#msgMas]) / 8 < $keyLen) { $keyMas[$#keyMas] = substr($keyMas[$#keyMas], 0, length(unpack "B*", $msgMas[$#msgMas]) / 8); }
